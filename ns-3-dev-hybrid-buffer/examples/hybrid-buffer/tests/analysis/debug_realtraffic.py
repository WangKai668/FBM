...
左下：
    Dr、ewmaRate、actualRate

右下：
    drop_real

python3 debug_realtraffic.py /home/wk/FBM/ns-3-dev-hybrid-buffer/examples/hybrid-buffer/tests/data/pbs/tc2-09/hybrid-buffer-test-tc2-09.txt --port 4
python3 debug_realtraffic.py /home/sj/FBM1/ns-3-dev-hybrid-buffer/examples/hybrid-buffer/tests/data/BMS/tc2-09/0.2M/hybrid-buffer-test-tc2-09.txt --port 4
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="解析 ns-3 FBM 调试日志并绘制指定端口的状态曲线。"
    )
    parser.add_argument("log_file", type=Path, help="ns-3 输出日志文件路径")
    parser.add_argument("--port", type=int, required=True, help="需要统计的端口号")
    parser.add_argument(
        "-o",
        "--output",
        type=Path,
        default=None,
        help="输出图片路径，默认：fbm_port_<端口>.png",
    )
    parser.add_argument(
        "--time-unit",
        choices=("ns", "us", "ms", "s"),
    ...
    parser.add_argument(
        "--rate-unit",
        default="log units",
        help="右侧速率轴单位名称，例如 Gbps，默认 log units",
    )
    parser.add_argument("--title", default=None, help="整张图的自定义标题")
    parser.add_argument("--dpi", type=int, default=200, help="输出图片 DPI，默认 200")
    parser.add_argument("--show", action="store_true", help="保存后弹出图形窗口")
    return parser
...
def main() -> int:
    ...
    if args.rate_divisor <= 0:
        print("错误：--rate-divisor 必须大于 0。", file=sys.stderr)
        return 1

    output_path = args.output or Path(f"fbm_port_{args.port}.png")
