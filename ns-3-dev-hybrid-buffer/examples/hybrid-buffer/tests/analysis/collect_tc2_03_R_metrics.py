...
def read_last_csv_field(csv_path: Path) -> float:
    ...
    try:
        ...
        raise ValueError(
            f"丢包文件最后一列不是数字：{csv_path}，"
            f"内容={last_row[-1]!r}"
        ) from exc


def calculate_avg_fct_ms(
    ...
    try:
        ...
        print(
            f"警告：读取FCT文件失败：{flow_analysis_path}，"
            f"原因：{exc}",
            file=sys.stderr,
        )
        return None
    ...
    if dataframe.empty:
        print(
            f"警告：FCT文件没有数据：{flow_analysis_path}",
            file=sys.stderr,
        )
        return None
