...
main(int argc, char* argv[])
{
    ...
    // isWeb为1时读取WebSearch流量，否则读取Hadoop流量
    ...
    if (lines.empty())
    {
        std::cerr << "错误：流量文件中没有可读取的内容：" << filename<< std::endl;
        return 1;
    }
    // 第一行是流量条数
    ...
    for (auto i : lines)    {
        ...
        if (isIncast){
            while (nowT + interval < std::stod(words.at(2)) - 2)
            {
                ...
                for (int k = numSpokes-10; k < numSpokes; k++) 
                    simHelper.AddFlow((uint32_t)k,
                                    0,
                                    Seconds(nowT),
                                    Seconds(sim_time),
