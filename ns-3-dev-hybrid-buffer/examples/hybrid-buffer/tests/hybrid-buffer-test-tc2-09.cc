...
main(int argc, char* argv[])
{
    ...
    for (const auto& line : lines)
    {
        ...
        try
        {
            ...
            double startTime =
                std::stod(words.at(2)) - 2;

            double stopTime =
                std::stod(words.at(3)) - 2;

            std::string dataRateString =
                words.at(4);
            if (isIncast)
            {
                while (nowT + interval < startTime)
                {
                    double incastStartTime = nowT;
                    double incastStopTime = nowT + lastTime;

                    simHelper.AddFlow(
                        6,
                        0,
                        Seconds(incastStartTime),
                        Seconds(incastStopTime),
                        DataRate("300Gbps"));
                    nowT += interval;
                }
            }
