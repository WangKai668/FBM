...
/* static */
bool
QueueSize::DoParse(const std::string s, QueueSizeUnit* unit, uint64_t* value)
{
    ...
    { // Found non-numeric
        ...
        else if (trailer == "kB" || trailer == "KB")
        {
            // kilobytes
            *unit = QueueSizeUnit::BYTES;
            *value = static_cast<uint64_t>(r * 1000);
        }
        else if (trailer == "KiB")
        {
            // kibibytes
            *unit = QueueSizeUnit::BYTES;
            *value = static_cast<uint64_t>(r * 1024);
        }
        else if (trailer == "MB")
        {
            // MegaBytes
            *unit = QueueSizeUnit::BYTES;
            *value = static_cast<uint64_t>(r * 1000000);
        }
        else if (trailer == "MiB")
        {
            // MebiBytes
            *unit = QueueSizeUnit::BYTES;
            *value = static_cast<uint64_t>(r * 1048576);
        }
        else if (trailer == "p")
        {
            // packets
...
QueueSize::QueueSize(QueueSizeUnit unit, uint64_t value)
    : m_unit(unit),
      m_value(value)
{
