...
class QueueSize
{
  public:
     ...
     */
    QueueSize(QueueSizeUnit unit, uint64_t value);
    /**
  ...
  private:
     ...
     */
    static bool DoParse(const std::string s, QueueSizeUnit* unit, uint64_t* value);

    // Uses DoParse
    friend std::istream& operator>>(std::istream& is, QueueSize& size);

    QueueSizeUnit m_unit; //!< unit
    uint64_t m_value;     //!< queue size [bytes or packets]
};
