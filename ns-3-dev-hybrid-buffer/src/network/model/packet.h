...
class Packet : public SimpleRefCount<Packet>
{
  public:
     ...
     * Set the ready state.
     *
     * This method should be called when this packet is completely read
     * from the buffer.
     *
     * \param [in] status the status to be set
     */
    void SetMmuFetchStatus(bool status);

    /**
     * Get the ready state.
     *
     * \returns true if the packet is fetched from mmu
     */
    bool GetMmuFetchStatus() const;

    /**
     * Set the mmu used destination port.
     *
     * \param [in] port The port number.
     */
    void SetMmuUsedPort(uint32_t port);

    /**
     * Get the mmu used destination port.
     *
     * \returns The port number.
     */
    uint32_t GetMmuUsedPort() const;

    /**
     * Set the mmu used sq id.
     *
     * \param [in] port The sq id.
     */
    void SetMmuUsedSqId(uint32_t sqId);
    
    /**
     * Get the mmu used sq id.
     *
     * \returns The sq id.
     */
    uint32_t GetMmuUsedSqId() const;

    /**
     * Set the mmu used queue index.
     *
     * \param [in] qIndex The queue index.
     */
    void SetMmuUsedQueueId(uint32_t qIndex);

    /**
     * Get the mmu used queue index.
     *
     * \returns The queue index.
     */
    uint32_t GetMmuUsedQueueId() const;

    /**
     * Set the mmu used priority.
     *
     * \param [in] prior The priority.
     */
    void SetMmuUsedPriority(uint32_t prior);

    /**
     * Get the mmu used priority.
     *
     * \returns The priority.
     */
    uint32_t GetMmuUsedPriority() const;

    /**
     * \brief The Packet Location Enum Type.
     */
    typedef enum
    {
        NOTINBUFFER,           //!< The Packet not in Buffer, are in some places
                               //!< like channel or ingress netdeive...
        OFFCHIPBUFFER,         //!< The Packet are in OFFCHIP DRAM.
        ONCHIPBUFFER,          //!< The Packet are in ONCHIP Buffer.
        WCACHE,                //!< The Packet are in WCACHE.
        WRITINGTOOFFCHIPBUFFER //!< The Packet are part in WCache Part in OffChipBuffer.
    } Location;

    /**
     * \brief Set the which buffer the packet is in
     *
    ...
    /**
     * \brief Get the location of this Packet.
     *
    ...
    /**
     * Add the Dram Stored Size
     *
     * \param size The size that add to dram stored.
     */
    void AddMmuDramStoredSize(uint32_t size);

    /**
     * Subtract the Dram Stored Size
     *
     * \param size The size that subtracted.
     */
    void SubtractMmuDramStoredSize(uint32_t size);

    /**
     * Get the Dram Stored Size
     *
     * \returns Return the Packet Dram Stored Size.
     */
    uint32_t GetDramStoredSize() const;
  ...
  private:
    ...
    Location m_location;           //!< the Packet Location;
    ...
    /* State used in reorder model */
    bool m_mmuFetchStatus; //!< the state to indicate whether this packet is fetched from mmu

    /* State used in switch mmu */
    uint32_t m_mmuDstPort;  //!< the destination port in passing switch
    uint32_t m_mmuQueueId;  //!< the queue index in passing switch
    uint32_t m_mmuPriority; //!< the priority in passing switch
    uint32_t m_mmuSqId;     //!< the sq index in passing switch

    uint32_t m_mmuDramStoredSize; //!< The Packet Size which stored in dram.
};
