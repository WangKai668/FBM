...
void SwitchMmu::CountDramBandwidth(){
    ...
    Dr_EWMA = Dr_EWMA * 0.9 + std::max(0.0, (m_offChipBuffer->GetDramBandwidth() - WriteDram_Rate_Total - ReadDram_Rate_Total)) * 0.1; //m_offChipBuffer->GetDramBandwidth() 空指针
    Simulator::Schedule(NanoSeconds(Dram_Bandwidth_Timer), &SwitchMmu::CountDramBandwidth, this);
}
...
SwitchMmu::BmResult SwitchMmu::Check3DTBmAlgorithm(Ptr<Packet> packet) { // FBM
    NS_LOG_FUNCTION(this << packet);
    /*
    ...
    */
    ...
    auto printPacketAdmissionResult = [&](bool intendedStoreSram){
        ...
        if (bmResult == BmResult(DROP)){
            if  (intendedStoreSram){
                ...
                if (QiS + static_cast<double>(pktSize) > dtThreshold)
                    cout << " DT阈值丢包:  Qis + pktSize:" << QiS + pktSize<< "| DT_Threshold:" << dtThreshold  << endl;
                else
                    cout <<" 原因未知"<< endl;
            }
            else
    ...
    if (cycleTime > 2*RTT || T_seq[port][priority][qIndex] <= 1){
    ...
    ///**************** 第N个(N>1)周期内 ******************/
        ...
        else if (QiS + pktSize <= dtThreshold && pktSize <= m_onChipBufferRemain && qlen + pktSize <= dtThreshold){ 
            bmResult = BmResult(TO_ONCHIPBUFFER);
    ...
    ///**************** 第N个(N>1)周期末 ******************/
        ...
        U2Ss = - std::min((Packet_Size_Cycle[port][priority][qIndex] - ReadSram_Size_Cycle[port][priority][qIndex]) * 1.0 
                            / Sr_last[port][priority][qIndex] + 1.0 * qlen_last[port][priority][qIndex] / S, 10.0);
        U2Ds = -std::min(lambdaLast / (Dr_last[port][priority][qIndex] +  WriteDram_Rate_Cycle_last[port][priority][qIndex]), 10.0);
    ...
    if (cycleTime > 2*RTT || T_seq[port][priority][qIndex] <= 1 || 
        ...
        if (print_flag == 1){
            ...
            cout << "DebugFBM: " << " time:" << Simulator::Now().GetNanoSeconds() 
                ...
                << "  newT:" << newT
                << "  Decision(0片外,1片内):" << newStoreDecision << "  bmResult(2丢包):" << bmResult
                << "  Usram:" << U_sram << "  Udram:" << U_dram
                << endl;
            cout << "  (1) BufferStates: " 
                    << " Qi:" << qlen/1e6 << "  QiS:" << QiS/1e6 << "  QiD:" << QiD/1e6 << "  mixed:" << isMixed
                    << "  Sr:" << Sr/1e6 << "  Dr:" << Dr/1e9 << "  DT:"<<dtThreshold/1e6 <<"  wcacheUsed/Size:"<<wcacheUsed/1e6<<"/"<<wcacheSize/1e6
                    << endl;
            cout << "  (2) RateStates: " 
