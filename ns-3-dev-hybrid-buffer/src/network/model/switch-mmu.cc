...
SwitchMmu::SetNQueues(uint32_t nQueues)
{
    ...
    for (uint32_t i = 0; i < m_nPorts; i++)
    {
        ...
        for (uint32_t j = 0; j < m_nPriorities; j++)
        {
            ...
            EWMA_Rslow[i][j].resize(m_nQueuesPerPort, 0);
...
SwitchMmu::SetNPriorities(uint32_t nPriorities)
{
    ...
    for (uint32_t i = 0; i < m_nPorts; i++)
    {
        ...
        EWMA_Rslow[i].resize(m_nPriorities);
...
SwitchMmu::CheckDeepHirBmAlgorithm(Ptr<Packet> packet) // 该函数用于检查基准缓存管理算法，根据输入的数据包信息和当前交换机状态，确定输入数据包的缓存位置
{
    ...
    const bool hasWcacheSpace = (wcacheSize - wcacheUsed) > pktSize;
    ...
    if (print_flag == 1 && port < 16 && cycleTime >= 10*1e3) // 每个10us打印一次
    {
        // 当前队列总占用
        ...
        cout << " (1) BufferStates: "
             ...
             << " Sr: "<< static_cast<double>(m_onChipBufferRemain) / 1e6 << " Dr: "<< Dr_EWMA
...
SwitchMmu::BmResult SwitchMmu::Check3DTBmAlgorithm(Ptr<Packet> packet) { // FBMM
    ...
    */
    ...
    bool isMixed = QiS > pktSize && QiD > pktSize;
    ...
    double lambdaEwmaLast = EWMA_Rslow[port][priority][qIndex];
    bool isBurst = lambdaDelta > LINK_BW * 2; // 判断是否为突发流量
    ...
    // }
        ...
        EWMA_Rslow[port][priority][qIndex] =  0.99 * EWMA_Rslow[port][priority][qIndex] + (1 - 0.99) * lambdaLast; //  平滑到达速率
        lambdaEwma = EWMA_R[port][priority][qIndex];
        // std::cout<<" ewma_new:"<<lambdaEwma  << std::endl;
        ...
        U2Ss = (Sr_last[port][priority][qIndex] - (Packet_Size_Cycle[port][priority][qIndex] - ReadSram_Size_Cycle[port][priority][qIndex])) * 1.0 / S
                - 0.5 * qlen_last[port][priority][qIndex] / S;
        ...
        U_sram_2 = (Sr - deltaQiS) * 1.0 / S - 0.5 * qlen / S;
        ...
        if (isMixed) {  // 队列已经混合时，不允许改变当前方向。
            ...
            if (storeDecision[port][priority][qIndex] == 0 && newStoreDecision){ //如果想从DRAM切到SRAM，需要满足
                // if (QiD / LINK_BW * lambdaEwma < Sr && isBurst )//LINK_BW / lambdaEwma < QiD / Sr){//outBytesMax < QiD ){ // 上一个周期存DRAM, 并且下一个周期连原有 DRAM 数据都发不完，SRAM 没有机会输出，继续存入 DRAM
                // if (lambdaDelta * RTT / 8.0 + qlen / LINK_BW * lambdaEwmaLast < Sr && isBurst )//LINK_BW / lambdaEwma < QiD / Sr){//outBytesMax < QiD ){ // 上一个周期存DRAM, 并且下一个周期连原有 DRAM 数据都发不完，SRAM 没有机会输出，继续存入 DRAM
                // if (isBurst || QiD / LINK_BW * lambdaEwma < Sr)//LINK_BW / lambdaEwma < QiD / Sr){//outBytesMax < QiD ){ // 上一个周期存DRAM, 并且下一个周期连原有 DRAM 数据都发不完，SRAM 没有机会输出，继续存入 DRAM
                if (isBurst || QiD / LINK_BW * lambdaEwma < Sr)//LINK_BW / lambdaEwma < QiD / Sr){//outBytesMax < QiD ){ // 上一个周期存DRAM, 并且下一个周期连原有 DRAM 数据都发不完，SRAM 没有机会输出，继续存入 DRAM
                    newStoreDecision = 1;
                else
                    newStoreDecision = 0;
    ...
    if ((cycleTime > 2*RTT && qlen == 0) || T_seq[port][priority][qIndex] <= 1 ||
        ...
        if (print_flag == 1 && port<12){
            ...
            cout << "  (4) T_Calculation: "  << " deltaU:" << deltaU << "  MD:" << MD 
                ...
                <<" drop_real/total_arrival:" << drop_real_per_period[port][priority][qIndex] << "/" << Packet_Size_Cycle[port][priority][qIndex]/pktSize <<" pktSize:" << pktSize
                << endl;
            cout << "  (5) DecisionStates: " 
                ...
                << " Qi/mu:(ns)" << qlen / LINK_BW * 8 << " Qi/mu*lambda<Sr:(KB)"<< qlen  / LINK_BW  * lambdaEwma * 1e-3 << " isBurst:" <<(lambdaDelta > LINK_BW * 2)
                <<" Qi/mu*lambdaEwma + lambdaDelta*RTT:"<< (lambdaDelta * RTT / 8.0 + qlen / LINK_BW * lambdaEwmaLast ) * 1e-3
                << " EWMA_Rslow:"<<EWMA_Rslow[port][priority][qIndex] <<" lambdaDelta:"<< lambdaDelta
                << endl;   
            cout << "--------------------------------------------------------------------------------"<<endl<<endl;    
        }
