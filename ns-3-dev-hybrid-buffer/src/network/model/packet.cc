...
Packet::Packet(const Packet& o)
    : m_buffer(o.m_buffer),
      ...
      m_mmuDramStoredSize(o.m_mmuDramStoredSize)
{
...
void
Packet::SetMmuFetchStatus(bool status)
{
    NS_LOG_FUNCTION(this);
    m_mmuFetchStatus = status;
}

bool
Packet::GetMmuFetchStatus() const
{
    NS_LOG_FUNCTION(this);
    return m_mmuFetchStatus;
}

void
Packet::SetMmuUsedPort(uint32_t port)
{
    NS_LOG_FUNCTION(this);
    m_mmuDstPort = port;
}
...
void
Packet::SetMmuUsedQueueId(uint32_t qIndex)
{
    NS_LOG_FUNCTION(this);
    m_mmuQueueId = qIndex;
}
...
uint32_t
Packet::GetMmuUsedQueueId() const
{
    NS_LOG_FUNCTION(this);
    return m_mmuQueueId;
}
...
void
Packet::SetMmuUsedSqId(uint32_t sqId)
{
    NS_LOG_FUNCTION(this);
    m_mmuSqId = sqId;
}
...
void
Packet::SetMmuUsedPriority(uint32_t prior)
{
    NS_LOG_FUNCTION(this);
    m_mmuPriority = prior;
}
...
uint32_t
Packet::GetMmuUsedPriority() const
{
    NS_LOG_FUNCTION(this);
    return m_mmuPriority;
}

void
Packet::SetLocation(Location location)
{
    NS_LOG_FUNCTION(this);
    m_location = location;
}
...
void
Packet::AddMmuDramStoredSize(uint32_t size)
{
    NS_LOG_FUNCTION(this);

    // TODO: It may need a Assert here, but for mmu input, there will
    // be packet fix size(add up packet header), so this assert may
    // be often violated.
    // NS_ASSERT(m_mmuDramStoredSize + size <= GetSize());
    m_mmuDramStoredSize += size;
}

void
Packet::SubtractMmuDramStoredSize(uint32_t size)
{
    NS_LOG_FUNCTION(this);
    NS_ASSERT(m_mmuDramStoredSize - size >= 0);

    m_mmuDramStoredSize -= size;
}

uint32_t
Packet::GetDramStoredSize() const
{
    NS_LOG_FUNCTION(this);

    return m_mmuDramStoredSize;
}
