...
SwitchMmu::SwitchMmu() // 构造函数SwitchMmu::SwitchMmu()被定义,来完成对象的初始化工作
{
    ...
    cout << "test1: " << baseFilePath << " " << nextFilePath << " " << baseFilePath + nextFilePath
         << endl;
...
SwitchMmu::SetNQueues(uint32_t nQueues)
{
    ...
    for (uint32_t i = 0; i < m_nPorts; i++)
    {
        ...
        for (uint32_t j = 0; j < m_nPriorities; j++)
        {
            ...
            m_Cost_ETC[i][j].resize(m_nQueuesPerPort, NanoSeconds(1000));
...
SwitchMmu::Check3DTBmAlgorithm(Ptr<Packet> packet) // PBS-new   //kkkk
{
    ...
    // cout<<"test "<<pktSize<<" "<<Packet_Size_Cycle[port][priority][qIndex]<<endl;
    ...
    double D = m_offChipBuffer->GetDramBandwidth();
    ; // Gbps Dram总带宽

    double Sr = m_onChipBufferRemain; // Sram 剩余大小
    // double Dr = 1000 - WriteDram_Rate_Cycle[port][priority][qIndex] -
    // ReadDram_Rate_Cycle[port][priority][qIndex]; 瞎写
    // 这个值怎么得到啊，只有这个端口的DRAM带宽使用量，没有整个交换机的，Gbps double Dr =
    // (wcacheSize - wcacheUsed)/wcacheSize * D;  //继续瞎写，用wcacheSize估一下DRAM剩余带宽
    // 不太行，要么就是满要么就是空，wcache反映不了剩余带宽

    double Dr = max(0.0, D - WriteDram_Rate_Total - ReadDram_Rate_Total); // Dram剩余带宽

    if (cycle_time > 2 * math_mETC && math_mETC > 1000)
    { // 1.处理一个端口多轮流量的情况，新一轮流量到达时要重置一些状态变量

        Predict_Flag_First[port][priority][qIndex] = 1;

        m_Cost_ETC[port][priority][qIndex] = NanoSeconds(1000);
             ...
             Predict_Flag_First[port][priority][qIndex] != 1) // 每个周期内
    {
        if (YRF_Flag_result[port][priority][qIndex] == true) // 存片内
        {
            ...
            // if (Pqs + pktSize <= DT_Threshold && m_onChipBufferRemain >= pktSize)
            // { // 如果DT阈值够且 SRAM存的下就存SRAM, 存不下就存DRAM，实在不行才丢
            //     bmResult = BmResult(TO_ONCHIPBUFFER);
            //     // 加入Ctag
            //     Ctag.insert(map<uint64_t, std::pair<int, size_t>>::value_type(
            //         packet->GetUid(),
    ...
    { // 每个周期结束 //周期末更新状态变量
        ...
        // double U1 = max(0.0, (Pqs_last[port][priority][qIndex] +
        // Packet_Size_Cycle[port][priority][qIndex] - DT_alpha*Sr)/1500); double U2 =
        // ((Sr_last[port][priority][qIndex] - Packet_Size_Cycle[port][priority][qIndex]/2.0)/S
        // + min(1.0,Dr_last[port][priority][qIndex]/D)) * math_mETC; U_star = U1 -
        // eta[port][priority][qIndex] * U2;
        U1Ss =
            1 / max(1.0,
        ...
        // U1Ds = 1.0 / max(1, drop_real_per_period[port][priority][qIndex]);

        // drop_DRAM_last[port][priority][qIndex] = max(0.0, (ewma_r - Dr) * new_T / 8.0 / 1e3 /
        // 1000);
        ...
        // double AI1 = Sr / Dr / 2 * 8.0;

        double AI1;

        /*
        if (temp_for_record_last_T < 7 * 1e3)
            AI1 = 1 * 1e3;
        else if (temp_for_record_last_T < Sr / Dr * 3 / 2 * 1e3)
            AI1 = 1.5 * Sr / Dr * 8.0;
        else
            AI1 = 5 * 1e3;
        ...
        */
        ...
        // AI1 = AI1 * drop_flag[port][priority][qIndex];
        // AI[port][priority][qIndex] = AI1[port][priority][qIndex];

        cout << " wktesttttt drop_flag " << drop_flag[port][priority][qIndex] << " AI "
             << AI[port][priority][qIndex] << endl;
        // double MD = 1.0 / (1 + (fabs(U_star - last_utility)));
        double delta_U = fabs(U_star - last_utility);
        // MD[port][priority][qIndex] =
        //     delta_U > 0.2 ? 0.075 / (0.1 + delta_U) + 0.25 : -2.5 * delta_U + 1;

        double cA = -0.5; // 控制幅度
        double cB = 20;   // 控制频率
        double cC = -4;   // 控制水平平移
        double cD = 0.5;  // 控制垂直平移
        MD[port][priority][qIndex] = cA * tanh(cB * delta_U + cC) + cD;
        // cout << "testkkkk " << -0.5 * tanh(20 * 0.000358105 - 4) + 0.5 << endl;
        // double MD1 = MD[port][priority][qIndex];
        ...
        // cout << "wktestdsfsdfsdf " << S << "、 " << D << "、 " << Tmax << endl << endl;
        m_Cost_ETC[port][priority][qIndex] =
            NanoSeconds(max(1e3, min(math_mETC + AI1, Tmax) * MD1));
        ...
        cout << Simulator::Now().GetNanoSeconds() << " New_Period T(ns):" << new_T
             ...
             << " |U-U*|: " << fabs(U_star - last_utility) << endl;
        ...
        // eta[port][priority][qIndex] = eta[port][priority][qIndex] * ( Sr/S + Dr/D );
        eta[port][priority][qIndex] =
            eta_base * (Sr / S + Dr / D); // 改一下 eta的基底固定为1e-5，然后用Sr/S+Dr/D来缩放
        // double eta_new = eta[port][priority][qIndex] * ( Sr/S + Dr/D );

        // η^∗(t+1)=η∗[Sr(t)  /S+Dr (t) /D]

        // QIS变化量 这里化简了一下不然会数据溢出 表达式应该是 new_T *（1e-9） * (100 * 1024 *
        // 1024
        // * 1024) / 8;
        ...
        U_sram_1 = 1 / max(1.0, (Pqs + delta_Q_i_S - DT_alpha * (Sr - delta_Q_i_S)) / 1e3 / 1000);
        ...
        /*
           ...
           {
               U_dram_1 =
                   1 / max(1.0, (ewma_r - Dr) * new_T / 8.0 / 1e3 / 1000); // Gbps*ns / 8 = B
           /1e6=MB U_dram_2 = -1.0 * ewma_r / Dr;                              // Gbps*ns / 8 = B
           }
...
SwitchMmu::FindBufferLocation(Ptr<Packet> packet)
{
    ...
    if (timermicro >= 1.0)
    {
        ...
        cout << Simulator::Now().GetNanoSeconds() << " 当前累积丢包率： "
             << (static_cast<double>(m_stats.nTotalBmDropPackets) /
                 (m_stats.nTotalStoredPackets + m_stats.nTotalBmDropPackets)) *
                    100.0
             << "%" << endl;
