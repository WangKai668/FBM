...
BulkSendApplication::SetMaxBytes(uint64_t maxBytes)
{
    ...
    m_maxBytes = maxBytes;
    // std::cout<<"debugwk-in-bulk-send-application setMaxBytes "<<m_maxBytes<<std::endl;
}
...
{
    NS_LOG_FUNCTION(this);
    // std::cout<<"debugwk-in-bulk-send-application SendData "<< Simulator::Now()<<" maxBytes:"<<m_maxBytes <<" m_sendSize: "<<m_sendSize<<" m_totBytes:"<<m_totBytes<<std::endl;
