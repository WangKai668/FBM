...
TcpSocketBase::SetInitialCwnd(uint32_t cwnd)
{
    ...
    // std::cout<<"debugwk: tcpSocketBase-InitialCwnd "<<cwnd<<std::endl;
}
...
TcpSocketBase::EnterRecovery(uint32_t currentDelivered)
{
    ...
    DoRetransmit();
    // std::cout<<"debugwk-intcp-socket-base: doretransmit: "<<m_dupAckCount << " dupack. Enter fast recovery mode."
    //                               << "Reset cwnd to " << m_tcb->m_cWnd << ", ssthresh to "
    //                               << m_tcb->m_ssThresh << " at fast recovery seqnum " << m_recover
    //                               << " calculated in flight: " << bytesInFlight<<std::endl;
    // (4.4) Run SetPipe ()
...
TcpSocketBase::AdvertisedWindowSize(bool scale) const
{
    ...
    if (w > m_maxWinSize)
    {
        ...
        NS_LOG_WARN("Adv window size truncated to "
                    ...
                    // std::cout<<"Adv window size truncated to "
                    // << m_maxWinSize <<" w "<<w<< "; possibly to avoid overflow of the 16-bit integer"<<std::endl;
    }
...
TcpSocketBase::SetSndBufSize(uint32_t size)
{
    ...
    // std::cout<<"debugwk-in-tcp-socket-base SteSendBufSize() "<<size <<std::endl;
}
