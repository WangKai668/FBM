...
{
    NS_LOG_FUNCTION(this << tcb << segmentsAcked);
    // std::cout<<"debugwkNewReno: CongestionAvoidance "<< tcb->m_cWnd << " ssthresh " << tcb->m_ssThresh <<std::endl;
...
{
    NS_LOG_FUNCTION(this << tcb << segmentsAcked);
        // std::cout<<"debugwkNewReno: IncreaseWindow "<< tcb->m_cWnd << " ssthresh " << tcb->m_ssThresh <<std::endl;
...
TcpNewReno::GetSsThresh(Ptr<const TcpSocketState> state, uint32_t bytesInFlight)
{
    ...
    // std::cout<<"debugwkNewReno: GetSsThresh  cwnd:"<< state->m_cWnd <<" sshresh"<< state->m_ssThresh<<" inflight:"<<bytesInFlight<<std::endl;
