...
// 打印TCP状态（不需要Socket访问）
    std::cout<< "TCP State - " << Simulator::Now().GetSeconds()<<
                ...
                "CongState: " << TcpSocketState::TcpCongStateName[tcb->m_congState]<<std::endl;
...
TcpCubic::GetSsThresh(Ptr<const TcpSocketState> tcb, uint32_t bytesInFlight)
{
    ...
    std::cout<<"debugwk-in-cubic: GetssThresh: "<<Simulator::Now()<<" cwnd="<<segCwnd<< " segments in flight=" << bytesInFlight / tcb->m_segmentSize<<std::endl;
