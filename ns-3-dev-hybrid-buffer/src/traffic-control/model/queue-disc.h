...
class QueueDisc : public Object
{
  public:
    ...
    /**
     * \brief Get the layer identity of this queue disc.
     *
     * \returns the layer indentity
     */
    LayerId GetLayerId() const;
    /**
     * \brief Set the layer identity of this queue disc.
     *
     * \param the layer identity to be set
     */
    void SetLayerId(LayerId id);

    /**
     * \brief Get the hierarchical enqueue classes from the root queue disc to the
     *        child queue disc for the given packet.
     *
     * This method should be used before a packet is enqueued to the root queue disc
     * by mmu, then mmu can use these classes to check the admission.
     * \param item item that attempt to enqueue
     * \return a vector that holds the layer id and class id of each layer follow the
     *         enqueue path
     */
    std::vector<std::tuple<LayerId, int32_t>> GetEnqueueClasses(Ptr<QueueDiscItem> item);
