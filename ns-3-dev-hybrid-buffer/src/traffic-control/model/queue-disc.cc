...
QueueDisc::LayerId
QueueDisc::GetLayerId() const
{
    return m_layerId;
}

void
QueueDisc::SetLayerId(QueueDisc::LayerId id)
{
    m_layerId = id;
}

std::vector<std::tuple<QueueDisc::LayerId, int32_t>>
QueueDisc::GetEnqueueClasses(Ptr<QueueDiscItem> item)
{
    NS_LOG_FUNCTION(this << item);
    std::vector<std::tuple<LayerId, int32_t>> classes;
    int32_t cls = Classify(item);
    if (cls == PacketFilter::PF_NO_MATCH)
    {
        // TODO: Is it right? @Yuqi
        return classes;
    }
    LayerId id = GetLayerId();
    classes.push_back(std::make_tuple(id, cls));
    Ptr<QueueDisc> child = GetQueueDiscClass(cls)->GetQueueDisc();
    if (child->GetNPacketFilters() > 0)
    {
        std::vector<std::tuple<LayerId, int32_t>> subClasses = child->GetEnqueueClasses(item);
        classes.insert(classes.end(), subClasses.begin(), subClasses.end());
    }
    return classes;
}
