                             ...
                             NetDevice::PacketType packetType)
{
    ...
    // std::cout<<"debugwk-in-traffic-control-layer【Receive】: device "<< device<<" p:"<<p<<" protocol:"<<protocol<<" from:"<<from<<" to:"<<to<<" packetType:"<<packetType<<std::endl;
...
TrafficControlLayer::Send(Ptr<NetDevice> device, Ptr<QueueDiscItem> item)
{
    ...
    else
    {
        ...
        Ptr<SwitchMmu> mmu = m_node->GetObject<SwitchMmu>();
        if (mmu == nullptr)
        {
            // This node may be a host and does not need buffer management
            qDisc->Enqueue(item);
            qDisc->Run();
        }
        else
        {
            // Buffer management from mmu before enqueue
            uint32_t port = 0;
            uint32_t qIndex = 0;
            uint32_t prior = 0;
            uint32_t sq = 0;

            std::vector<std::tuple<QueueDisc::LayerId, int32_t>> cls;
            cls = qDisc->GetEnqueueClasses(item);
            for (uint32_t i = 0; i < cls.size(); i++)
            {
                switch (std::get<0>(cls[i]))
                {
                case QueueDisc::sq:
                    sq = std::get<1>(cls[i]);
                    break;
                case QueueDisc::priority:
                    prior = std::get<1>(cls[i]);
                    break;
                case QueueDisc::dstPort:
                    port = std::get<1>(cls[i]);
                    break;
                case QueueDisc::queue:
                    qIndex = std::get<1>(cls[i]);
                    break;
                default:
                    break;
                }
            }
            port = device->GetIfIndex();
            Ptr<Packet> pkt = item->GetPacket();
            pkt->SetMmuUsedPort(port);
            pkt->SetMmuUsedQueueId(qIndex);
            pkt->SetMmuUsedPriority(prior);
            pkt->SetMmuUsedSqId(sq);

            SwitchMmu::BmResult result = mmu->CheckIngressAdmission(pkt);
            if (result != SwitchMmu::DROP)
            {
                mmu->Store(pkt, result);
                qDisc->Enqueue(item);
                qDisc->Run();
            }
            else{
                // std::cout<<"debugwk-in-traffic-control-layer: "<< "drop"<<std::endl;
            }
        }
    }
}
